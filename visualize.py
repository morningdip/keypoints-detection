...
def display_keypoints(image, boxes, keypoints, class_ids, class_names,
                      skeleton=None, scores=None, title="", figsize=(16, 16), ax=None):
    """
    ...
    for i in range(N):
        ...
        for Joint in keypoints[i]:
            if(Joint[2] != 0):
                circle = patches.Circle((Joint[0], Joint[1]), radius=3, edgecolor=(
                    0, 1, 0), facecolor='none', linewidth=3)
                ax.add_patch(circle)
                keypoints_count += 1
            ...
            for connection in skeleton:
                ...
                if ((Joint_start[2] != 0) & (Joint_end[2] != 0)):
                    cv2.line(skeleton_image, tuple(Joint_start[:2]), tuple(
                        Joint_end[:2]), [x * 255 for x in color])
...
def display_top_masks(image, mask, class_ids, class_names, limit=4):
    ...
    top_ids = [v[0] for v in sorted(zip(unique_class_ids, mask_area), key=lambda r: r[
                                    1], reverse=True) if v[1] > 0]
